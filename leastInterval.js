...
const leastInterval = (tasks, n) => {
    ...
    for (let key in store) {
        sorted.push([key, store[key]])
    }

    sorted.sort((b,a) => a[1] - b[1])
    const newStore = {}
    for (let element of sorted) {
        newStore[element[0]] = element[1]
    }

    const maxVal = newStore[Object.keys(newStore)[0]] -1
